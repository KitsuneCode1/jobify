...
import dashboard1 from "../assets/dashboard-icon-01.svg";
import dashboard2 from "../assets/dashboard-icon-02.svg";
...
const Dashboard = () => {
  ...
  const navItems = [
    ...
    { name: "Saved Candidate", icon1: save1, icon2: save2 },
    { name: "Membership", icon1: bag1, icon2: bag2 },
  ...
  return (
    <>
      <div className="dashboard-wrapper relative w-full flex items-start justify-between bg-[#f0f5f3] min-h-[100vh]">
        <div
          className={`sidebar bg-white shadow-lg pt-[30px] px-[20px] ${isSidebarActive ? "active" : ""}`}
        >
          <img
            src={menuclose}
            alt="menu-close"
          ...
          <div className="admin-info text-center mb-10">
            ...
            <div>
              ...
              >
                John Smith
                <ul
                  className={`dropdown-menu absolute left-1/2 -translate-x-1/2 mt-2 w-[240px] bg-white rounded-md shadow-md p-5 space-y-3 transition-all duration-300 ease-in-out z-10 ${isOpen ? "opacity-100 visible translate-y-0" : "opacity-0 invisible -translate-y-2"}`}
                >
                  <li>
                    <a
                      href="#"
          ...
          <ul className="dashboard-main-nav">
            <li>
              <Link
                ...
                className={`flex items-center w-full gap-2 p-4 rounded-2xl ${activeItem === "Dashboard" ? "active" : ""}`}
              >
                <img
                  src={dashboard1}
                  alt="dashboard-icon"
                  className={`w-[18px] transition-opacity duration-300 ease-in-out ${activeItem === "Dashboard" ? "opacity-0 absolute" : "opacity-100 relative"}`}
                />
                <img
                  src={dashboard2}
                  alt="dashboard-icon"
                  className={`w-[18px] transition-opacity duration-300 ease-in-out ${activeItem === "Dashboard" ? "opacity-100 relative" : "opacity-0 absolute"}`}
                />
                <span className="text-md ps-2 font-[500]">Dashboard</span>
              </Link>
            </li>
            {navItems.map((item) => (
              <li key={item.name}>
                <Link
                  ...
                  className={`flex items-center w-full gap-2 p-4 rounded-2xl ${activeItem === item.name ? "active" : ""}`}
                >
                  <img
                    src={item.icon1}
                    alt={`${item.name}-icon1`}
                    className={`w-[18px] transition-opacity duration-300 ease-in-out ${activeItem === item.name ? "opacity-0 absolute" : "opacity-100 relative"}`}
                  />
                  <img
                    src={item.icon2}
                    alt={`${item.name}-icon2`}
                    className={`w-[18px] transition-opacity duration-300 ease-in-out ${activeItem === item.name ? "opacity-100 relative" : "opacity-0 absolute"}`}
                  />
                  <span className="text-md ps-2 font-[500]">{item.name}</span>
                </Link>
              </li>
            ))}
            ...
            <li>
              ...
              >
                ...
                <span className="text-md hover:text-red-500">Delete</span>
              </button>
            </li>
          </ul>
              ...
              <div className="bg-white p-12 rounded-xl text-center w-full max-w-xl mx-auto shadow-lg">
                ...
                <div className="flex justify-center gap-6">
                  ...
                  <button
                    ...
                    className="text-[#1c6c50] hover:underline font-[500] text-lg"
                  >
                    Cancel
                  </button>
                </div>
        ...
        {/* Main */}
        <div className="main bg-[#f0f5f3] min-h-full h-full w-full ml-[265px] overflow-y-scroll">
          <div className="nav-main bg-[#f0f5f3] w-full h-[90px] flex items-center justify-end gap-4 z-[9]">
            ...
            <form className="search-form w-[300px] mr-6 relative border-none hidden lg:flex">
              ...
              <input
                ...
                className="w-full px-10 rounded-full font-[300] bg-[rgba(0,0,0,0.05)] text-black border-none shadow-none focus:outline-none pl-[45px] pr-[15px] h-[45px] text-base"
              />
            </form>
            <button
            ...
            >
              ...
              <div
                className={`absolute left-[-150px] top-[40px] text-start mt-2 w-[250px] bg-white rounded-xl shadow-md p-5 space-y-4 z-10 transition-all duration-300 ease-in-out ${isNotification ? "opacity-100 visible translate-y-0" : "opacity-0 invisible -translate-y-2"}`}
              >
            ...
            <Link to="Pages/SubmitJob" className="bg-[#3f634d] text-white hover:bg-[#d2f34c] hover:text-[#244034] py-3 px-5 text-sm md:text-md font-[500] rounded-full transition duration-300" type="button">Post a Job</Link>
          </div>
